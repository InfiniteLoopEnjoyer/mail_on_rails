...
# docker container running for MailOnRails::ClamavScanner. The scanner's
# default address is the clamav Kamal accessory's container name, which only
# resolves on the deployed docker network - pointing dev at it would file
# every inbound message into Quarantine as "unscanned". So this plugin
# repoints SMTP_CLAMAV_ADDR at a local container it manages, or pins it to
# "" (scanning off, mail delivered unscanned) when docker is unavailable.
