...
module MailOnRails
  module Imap
    ...
    module TLS
      ...
      # healthy while dropping the IMAPS listener and (since LOGINDISABLED
      # holds until the channel is encrypted) refusing every login.
      ...
      def material(dir: nil, logger: nil)
        cert_path = Settings.static(:imap_tls_cert)
        key_path = Settings.static(:imap_tls_key)
      ...
      def hostnames
        hosts = Settings.static(:imap_tls_hosts).dup
