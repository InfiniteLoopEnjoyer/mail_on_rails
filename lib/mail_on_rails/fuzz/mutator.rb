...
module MailOnRails
  module Fuzz
    ...
    class Mutator
      ...
      # True once any call this round actually altered its input or emitted
      # garbage. Harnesses reset this per round and use it to decide whether
      # a dropped connection could be the server's legitimate reaction to a
      # mutated line (e.g. one mutated into "QUIT <junk>").
      def mutated? = @mutated

      def reset! = @mutated = false
