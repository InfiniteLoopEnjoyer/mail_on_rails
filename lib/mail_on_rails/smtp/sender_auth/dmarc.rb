...
# The full ICANN Public Suffix List for organizational-domain computation.
# Optional on purpose: the protocol tree must keep working in a bare
# process without the gem (the fallback table below takes over).
begin
    ...
    module SenderAuth
      ...
      class Dmarc
        ...
        def evaluate(from_domain:, spf:, dkim:)
          ...
          dkim_ok = dkim_aligned?(from_domain, dkim, tags.fetch("adkim", "r"))
          spf_ok = spf_aligned?(from_domain, spf, tags.fetch("aspf", "r"))
          aligned = dkim_ok || spf_ok

          if !aligned && transient_alignment_loss?(from_domain, spf, dkim, tags)
            return { result: :temperror, policy: :none, domain: record_domain, from_domain: from_domain }
          end

          {
            ...
            # Per-mechanism aligned verdicts and the published policy tags,
            # carried for the RFC 7489 aggregate reports (policy_evaluated
            # and policy_published blocks).
            dkim_aligned: dkim_ok,
            spf_aligned: spf_ok,
            published: { p: tags["p"], sp: tags["sp"],
                         adkim: tags.fetch("adkim", "r"), aspf: tags.fetch("aspf", "r"),
        ...
        # Fallback suffix table for when the public_suffix gem is absent: a
        # pragmatic subset of the Public Suffix List - common two-label
        # suffixes plus the default of "last two labels". Wrong for exotic
        # suffixes, but those only loosen relaxed alignment for domains we
        # are unlikely to see.
        ...
        # The registrable ("organizational") domain, computed against the
        # full ICANN Public Suffix List when the public_suffix gem is
        # loaded (its default wildcard rule already answers "last two
        # labels" for unlisted TLDs). PublicSuffix.domain returns nil for
        # names that have no registrable part (a bare suffix, an IP
        # literal); those and a missing gem fall back to the static table.
        def self.org_domain(domain)
          name = domain.to_s.downcase
          if defined?(PublicSuffix) && (registrable = PublicSuffix.domain(name))
            return registrable
          end
        ...
        # A :fail verdict is only trustworthy when no aligned mechanism was
        # lost to a transient DNS error: an SPF or DKIM check that
        # temperror'd on a domain which *could* have aligned might have
        # passed, so the evaluation is :temperror (RFC 7489 §11.2), not
        # :fail. Unaligned domains don't count - they could not have
        # changed the outcome, and counting them would let a spoofer
        # attach a DKIM signature under broken DNS to soften p=reject
        # into a deferral.
        def transient_alignment_loss?(from_domain, spf, dkim, tags)
          (spf && spf[:result] == :temperror && aligned?(from_domain, spf[:domain], tags.fetch("aspf", "r"))) ||
            Array(dkim).any? { |s| s[:result] == :temperror && aligned?(from_domain, s[:domain], tags.fetch("adkim", "r")) }
        end
