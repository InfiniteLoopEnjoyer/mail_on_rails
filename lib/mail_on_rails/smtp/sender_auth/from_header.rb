  ...
  module Smtp
    module SenderAuth
      ...
      module FromHeader
        ...
        # Every addr-spec in the named header, lowercased - the MSA
        # from-alignment check's view of who the message claims to be
        # from. nil when the header is absent; an unparseable header or
        # mailbox yields a nil entry (never silently dropped - the caller
        # must treat "cannot parse" as "cannot authorize").
        def addresses(data, field: "from")
          pattern = /\A#{Regexp.escape(field)}[ \t]*:/i
          fields = unfolded_headers(data).select { |f| f.match?(pattern) }
          return nil if fields.empty?

          fields.flat_map do |header|
            mailboxes = split_mailboxes(header.sub(pattern, ""))
            next [ nil ] unless mailboxes&.any?

            mailboxes.map { |mailbox| addr_spec(mailbox)&.downcase }
          end
        end
