...
module MailOnRails
  module Smtp
    ...
    module SenderAuth
      ...
      # Master switch for the verifiers, read at load time (in the app
      # these files load from the Puma plugin, with the environment already
      # set); per-listener spec[:sender_auth] overrides it (the test seam).
      # On by default - only "0" disables - for local try-outs where live
      # DNS lookups per message are unwanted.
