...
module MailOnRails
  class DnsPublisher
    ...
    # A staged rotation's next selector needs its TXT out before the key
    # can be promoted (RotateDkimKeysJob checks public DNS first).
    def publish_staged_dkim
      return unless @domain.dkim_staged?

      existing = txt_records(@domain.dkim_next_txt_name)
      expected = @domain.dkim_next_txt_value
      if existing.empty?
        @client.create_record(@zone, type: "TXT", name: @domain.dkim_next_txt_name, content: quoted_txt(expected), ttl: TTL)
        @actions << "created staged DKIM TXT (#{@domain.dkim_next_txt_name})"
      elsif existing.any? { |r| dkim_p(content_of(r)) == dkim_p(expected) }
        @skipped << "staged DKIM TXT already matches the staged key"
      else
        @client.update_record(@zone, existing.first["id"], type: "TXT", name: @domain.dkim_next_txt_name, content: quoted_txt(expected), ttl: TTL)
        @actions << "updated staged DKIM TXT to the staged key"
      end
    end
    ...
    # Self-asserted BIMI, only once the operator uploaded a logo for the
    # domain (Domain#bimi_svg). Never overwrites an existing record - a
    # VMC-backed record someone published by hand must win.
    def publish_bimi
      return if @domain.bimi_svg.blank?

      existing = txt_records(@domain.bimi_txt_name).find { |r| content_of(r).match?(/\Av=BIMI1\b/i) }
      if existing
        @skipped << "BIMI already published (#{content_of(existing)}) - not modifying an existing assertion"
      elsif web_host.blank?
        @skipped << "BIMI: MAIL_ON_RAILS_WEB_HOST is not set, nowhere to serve the logo from"
      else
        @client.create_record(@zone, type: "TXT", name: @domain.bimi_txt_name,
                                     content: quoted_txt(@domain.bimi_txt_value(web_host)), ttl: TTL)
        @actions << "created BIMI TXT (logo at https://#{web_host}#{@domain.bimi_logo_path})"
      end
    end
    ...
    # overflows a single one). Class method so RotateDkimKeysJob shares it.
    def self.quoted_txt(content)
      content.scan(/.{1,255}/m).map { |part| %("#{part}") }.join(" ")
    end

    def quoted_txt(content)
      self.class.quoted_txt(content)
    end
