module MailOnRails
  class EmailAccount < Record
    ...
    # Canary accounts (honeypot: true) are decoys no real user owns; a
    # successful login against one can only be an attacker. See HoneypotEvent.
    scope :honeypots, -> { where(honeypot: true) }
    scope :real, -> { where(honeypot: false) }
...
ActiveSupport.run_load_hooks :mail_on_rails_email_account, MailOnRails::EmailAccount
