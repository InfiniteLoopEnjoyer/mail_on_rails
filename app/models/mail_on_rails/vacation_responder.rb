...
module MailOnRails
  module VacationResponder
    ...
    def build_reply(account, original, recipient)
      ...
      reply.to      = recipient
      # The inbound Subject is attacker-controlled text landing in an
      # outbound header; strip CR/LF explicitly (the Mail gem usually
      # encodes, but this must not depend on it) - same rule as
      # Store::SmtpBackend#sanitize_header. The admin-entered
      # vacation_subject gets the same treatment for free.
      subject = account.vacation_subject.presence || "Automatic reply: #{original.subject}".strip
      reply.subject = subject.gsub(/[\r\n]/, " ")
