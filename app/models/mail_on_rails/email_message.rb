...
module MailOnRails
  class EmailMessage < Record
    ...
    # Turbo broadcasts attach via ActiveSupport.on_load(:mail_on_rails_email_message).

    # Bounds body_text at write time (in characters). On PostgreSQL the
    # generated search_vector column re-caps in SQL, but an unbounded
    # write could push the tsvector past Postgres's 1 MiB limit and turn a
    # huge inbound message into a failed delivery.
    SEARCHABLE_TEXT_LIMIT = 100_000

    # Full-text search over subject/from/to/body_text. On PostgreSQL it
    # hits the stored search_vector - websearch_to_tsquery gives "words,
    # "quoted phrases", OR, -exclusions" semantics and never raises on
    # user input. Other adapters use the portable substring fallback.
    scope :full_text_search, ->(query) {
      if MailOnRails::AdapterSupport.postgresql?
        where("#{table_name}.search_vector @@ websearch_to_tsquery('simple', ?)", query.to_s)
      else
        like_search(query)
      end
    }
    ...
    # Portable search for adapters without the generated tsvector: every
    # term must appear (case-insensitive substring) in at least one of the
    # searched columns. Substrings match more generously than PostgreSQL's
    # word-level semantics, which the store contract explicitly allows.
    # "!" is the explicit LIKE escape character - SQLite has no default
    # one and MySQL double-processes backslashes.
    def self.like_search(query, columns: %w[subject from_address to_addresses body_text])
      terms = query.to_s.split(/\s+/).reject(&:empty?).first(SEARCH_TERMS_LIMIT)
      return none if terms.empty?

      terms.reduce(all) do |scope, term|
        pattern = "%#{sanitize_sql_like(term.downcase, "!")}%"
        scope.where(columns.map { |column| "LOWER(#{table_name}.#{column}) LIKE :pattern ESCAPE '!'" }.join(" OR "),
                    pattern: pattern)
      end
    end
    ...
    def self.deliver_raw(mailbox, raw, flags: [], internal_date: nil, authenticated_as: nil, auth_results: nil,
      ...
      create = lambda do
        mailbox.email_messages.create!(
          ...
          flags: flags,
          # RFC 3501: INTERNALDATE is the delivery time, never the sender's
      ...
      # "move to Trash" on a full account would lock the user out of freeing
      # space; unlimited accounts skip the gate entirely (the hot path).
      #
      # The check and the create run under a row lock on the account so
      # concurrent deliveries can't both read "room for one more" and both
      # write: with_lock opens a transaction that holds the lock through the
      # after_create used_bytes increment, so the next waiter reloads the
      # updated total. (claim_uid!/claim_modseq! then lock the mailbox row -
      # always account-then-mailbox, so no lock-order cycle.)
      return create.call unless enforce_quota && account.quota_bytes.present?

      account.with_lock do
        if account.quota_exceeded_by?(raw.bytesize)
    ...
    def self.resolve_thread_id(account, references, in_reply_to, message_id, fallback_anchor)
      ...
      if ancestors.any?
        parent = joins(:mailbox).where(Mailbox.table_name => { email_account_id: account.id }, message_id: ancestors)
...
ActiveSupport.run_load_hooks :mail_on_rails_email_message, MailOnRails::EmailMessage
