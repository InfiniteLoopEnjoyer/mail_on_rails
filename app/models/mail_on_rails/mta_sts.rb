...
module MailOnRails
  class MtaSts
    ...
    def self.mode
      value = MailOnRails::Settings.static(:mta_sts_mode)
    ...
    def self.max_age
      # The schema types and range-checks an explicit value (env or
      # override); MAX_AGE_RANGE is enforced there via min/max. Re-raised
      # as ArgumentError - the boot-time validation initializer's
      # longstanding contract.
      MailOnRails::Settings[:mta_sts_max_age] || (mode == "enforce" ? 604_800 : 86_400)
    rescue MailOnRails::Netserv::Config::Error => e
