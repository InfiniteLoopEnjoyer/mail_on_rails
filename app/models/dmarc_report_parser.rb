...
class DmarcReportParser
  ...
  def ingest
    ...
    feedback = Nokogiri::XML(xml).at_xpath("/feedback")
    ...
    reporter = feedback.at_xpath("report_metadata/org_name")&.text.to_s.strip
    report_id = feedback.at_xpath("report_metadata/report_id")&.text.to_s.strip
    ...
    rows = feedback.xpath("record").filter_map do |record|
      ...
      { domain_id: domain.id, reporter: reporter, report_id: report_id,
        begin_at: begin_at, end_at: end_at, source_ip: source_ip,
        count: record.at_xpath("row/count")&.text.to_i.clamp(1, 10_000_000),
        disposition: record.at_xpath("row/policy_evaluated/disposition")&.text.to_s.strip.presence,
        dkim: record.at_xpath("row/policy_evaluated/dkim")&.text.to_s.strip.downcase.presence,
        spf: record.at_xpath("row/policy_evaluated/spf")&.text.to_s.strip.downcase.presence }
    end
