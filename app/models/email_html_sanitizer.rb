...
# page. This is the first of two independent layers: the host app's view
# should additionally render the result inside a sandboxed iframe (opaque
# origin, no scripts), so a sanitizer miss still cannot run code or touch
# the session.
#
# Deliberately top-level (not MailOnRails::): this is the seam
# EmailMessage#html_body calls, and a host app that needs a different
# policy overrides it by defining its own app/models/email_html_sanitizer.rb
# (the app's autoload paths shadow the engine's). Overrides must keep the
# contract: .sanitize(html, inline_images:, allow_remote_images:) returning
# a Result, and an InlineImage data type.
#
# Policy:
...
class EmailHtmlSanitizer
  ...
  def data_uri(image)
    "data:#{image.content_type};base64,#{[ image.data ].pack("m0")}"
  end
