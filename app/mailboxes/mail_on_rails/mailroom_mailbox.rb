...
# Delivers inbound mail into the INBOX of every local account named in the
# X-Original-To envelope headers stamped by the in-process SMTP server
# (Store::SmtpBackend#stamp). Routing follows the SMTP RCPT list only -
# To/Cc/Bcc headers inside DATA are display metadata and must not add
# recipients the edge never accepted (a forged Bcc must not reach another
# user's mailbox). SMTP Bcc is captured because every RCPT TO is stamped.
#
...
#   - Virus scanning via clamav (SMTP_CLAMAV_ADDR; the schema default is
#     "" = off, but production refuses to boot the mail servers like that
#     unless SMTP_CLAMAV_OPTIONAL=1 - see Runtime.require_virus_scanner!).
...
module MailOnRails
  class MailroomMailbox < ActionMailbox::Base
    def process
      ...
      recipients.filter_map { |recipient| resolve_account(recipient.strip.downcase) }.uniq.each do |account|
        ...
        if verdict && verdict[:status] != "clean"
          ...
          # Mail to a domain's dmarc@ / tls-rpt@ ingestion account carries
          # aggregate reports - parse them only after a real clamav scan
          # came back clean (on this branch a present verdict IS clean).
    ...
    # The trusted routing/auth headers are only believable with a valid
    # IngressSeal from the SMTP edge. An unsealed message reached this
    # mailroom by some route other than the edge (which should not exist -
    # the HTTP ingress is pinned 404) and its headers are unauthenticated.
    # When MAILROOM_REQUIRE_SEAL is on (the default) it is dropped (never
    # bounced - backscatter to a spoofable envelope sender helps no one);
    # the MAILROOM_REQUIRE_SEAL=0 opt-out processes it with a warning, for
    # the soak window of a sealing rollout only.
    def sealed_or_permitted?
      # max_age comes from the mailroom_seal_max_age setting so a backlog
      # incident is answered by raising the window, not by turning the
      # seal requirement off.
      return true if MailOnRails::IngressSeal.verify(inbound_email.source,
                                                     max_age: MailOnRails::Settings[:mailroom_seal_max_age])

      if MailOnRails::Settings[:mailroom_require_seal]
        Rails.logger.error "[mail_on_rails] dropping unsealed inbound email ##{inbound_email.id}: " \
                           "trusted headers are unauthenticated (MAILROOM_REQUIRE_SEAL is on)"
        false
      else
        Rails.logger.warn "[mail_on_rails] processing UNSEALED inbound email ##{inbound_email.id}: " \
                          "trusted headers are unauthenticated (MAILROOM_REQUIRE_SEAL=0 is set - " \
                          "remove the opt-out once the sealing deploy has soaked)"
        true
      end
    end
    ...
    # A recipient address is either an account's own address, one of its
    # aliases (the two sets are kept disjoint by model validations), or a
    # signed VERP bounce+ sub-address, which delivers into the domain's
    # bounce@ account (whose address then routes IngestBounceJob).
    def resolve_account(address)
      EmailAccount.find_by(email: address) || EmailAlias.find_by(email: address)&.email_account ||
        verp_account(address)
    end

    def verp_account(address)
      local, _, domain = address.partition("@")
      return nil unless local.start_with?("#{Domain::BOUNCE_LOCAL_PART}+") && VerpAddress.valid?(address)

      EmailAccount.find_by(email: "#{Domain::BOUNCE_LOCAL_PART}@#{domain}")
    end

    def recipients
      header_values("X-Original-To").map(&:to_s).map(&:strip).reject(&:empty?).uniq
    end
    ...
    def dmarc_policy_junk?
      ...
      @dmarc_policy_junk = begin
        ...
        mode = MailOnRails::Settings[:mailroom_dmarc_enforce].to_s.strip.downcase
