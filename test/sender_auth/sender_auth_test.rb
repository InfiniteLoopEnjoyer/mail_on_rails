...
class SenderAuthTest < Minitest::Test
  ...
  test "from domain extraction handles display names and is nil for multiple froms" do
    assert_equal "example.com", MailOnRails::SenderAuth.from_domain(MESSAGE)
    two_froms = MESSAGE.sub("From: Alice <alice@example.com>", "From: a@x.com, b@y.com")
    assert_nil MailOnRails::SenderAuth.from_domain(two_froms)
    assert_nil MailOnRails::SenderAuth.from_domain("Subject: no from\r\n\r\nhi\r\n")
  end

  test "enforcement is the default, disabled only by SMTP_DMARC_ENFORCE=0" do
    assert MailOnRails::SenderAuth.enforce_dmarc?
    ENV["SMTP_DMARC_ENFORCE"] = "0"
    assert_not MailOnRails::SenderAuth.enforce_dmarc?
  ensure
    ENV.delete("SMTP_DMARC_ENFORCE")
  end
end
