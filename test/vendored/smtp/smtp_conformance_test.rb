...
class SmtpConformanceTest < Minitest::Test
  ...
  # -- unadvertised extensions -----------------------------------------------

  def test_bdat_is_not_advertised_and_not_implemented
    with_session do |client|
      read_reply(client)
      ehlo = command(client, "EHLO client.test")
      refute_match(/CHUNKING/, ehlo)
      assert_match(/\A502/, command(client, "BDAT 10 LAST"))
      command(client, "QUIT")
    end
  end

  def test_smtputf8_parameter_is_refused_because_unadvertised
    with_session do |client|
      start_session(client)
