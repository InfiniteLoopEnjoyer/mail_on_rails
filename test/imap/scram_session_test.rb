...
class ScramSessionTest < Minitest::Test
  ...
  # The oracle the old code leaked was structural, not textual: a known
  # user got a "+" server-first challenge, an unknown one an immediate
  # tagged NO. Both must now reach the challenge, with a well-formed
  # salt/iteration count, so the two are indistinguishable up to the proof.
  test "an unknown account still receives a server-first challenge" do
    client = session
    server_first, _bare, raw = begin_exchange(client, "a1", user: "nobody@example.test")

    assert server_first, "unknown users must get a + challenge, not an immediate NO (got #{raw.inspect})"
    attrs = server_first.split(",").to_h { |p| [ p[0], p[2..] ] }
    assert attrs["s"], "the decoy challenge must carry a salt"
    assert_operator attrs["i"].to_i, :>=, 4096, "and a realistic iteration count"
  end

  test "the decoy salt for an unknown account is stable across connections" do
    salts = 2.times.map do
      client = session
      server_first, = begin_exchange(client, "a1", user: "ghost@example.test")
      server_first.split(",").to_h { |p| [ p[0], p[2..] ] }["s"]
    end

    assert_equal salts.first, salts.last, "a per-attempt-random salt would itself be an oracle"
  end
