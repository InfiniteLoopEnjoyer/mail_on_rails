...
# End-to-end runs of the full server stack - accept threads spawning one
# thread per connection - over real loopback sockets: the mailbox flow,
# STARTTLS and implicit TLS, concurrent sessions, IDLE not blocking other
# connections, and the connection cap with slot release.
class ServerDispatchTest < Minitest::Test
  ...
  def start_server(store, listeners, tls_material: nil, server_class: MailOnRails::ImapServer)
    ...
    thread = Thread.new { server_class.run(store, specs, tls_material) }
    @cleanup << -> { thread.kill }
    specs
  end
  ...
  def test_serves_sessions_concurrently
    spec = start_server(memory_store, [ :starttls ]).first
  ...
  # Regression (kept from the fiber-scheduler era): a client sitting in
  # IDLE must not block other sessions or new connections. Under
  # thread-per-connection the IDLE wait parks only its own thread, but the
  # observable guarantee is worth pinning either way.
  ...
  # IDLE_POLL_SECONDS on the server class is the settings-backed poll's
  # override seam (sub-second values the integer schema can't express).
  ...
  def test_connection_cap_and_release
