...
class SmtpVirusScanTest < Minitest::Test
  ...
  def test_infected_message_gets_550_and_a_quarantined_review_copy
    FakeClamd.serving(:infected) do |addr|
      ...
      assert_match(/\A550 5\.7\.1 .*virus/i, reply)
      # The signature name is an oracle for tuning payload packing - it
      # belongs in the log and the quarantine row, never in the reply.
      refute_match(/Eicar/i, reply)
    end
    ...
    assert_match(/\AReceived: from /, copy[:data])
    assert copy[:data].end_with?(RAW), "stored data must be our Received header + the payload"
  end
