...
class DmarcTest < Minitest::Test
  ...
  test "org domain uses the full public suffix list when the gem is loaded" do
    ...
    assert_equal "example.pvt.k12.ma.us", MailOnRails::Smtp::SenderAuth::Dmarc.org_domain("www.example.pvt.k12.ma.us")
    assert_equal "user.github.io", MailOnRails::Smtp::SenderAuth::Dmarc.org_domain("sub.user.github.io"),
                 "private-section PSL entries count, matching common DMARC implementations"
    # Unlisted TLDs keep the last-two-labels default.
    assert_equal "example.test", MailOnRails::Smtp::SenderAuth::Dmarc.org_domain("mail.example.test")
  end
