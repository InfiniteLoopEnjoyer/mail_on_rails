...
`live_info` fields (`user:, state:, tls:, helo:, messages:`). When the
...
### `banned_cidrs()` — optional

The admin ban list as an array of IP/CIDR strings (both protocols; the
SMTP server calls it too). The accept-side `Netserv::Denylist` polls it
at most once per its TTL and drops matching connections before any
banner. Optional: the server checks `respond_to?` at construction, so a
store without it leaves the denylist off. Fail-soft on the reader's
side: any non-array result (the AR store returns `db`'s error hash on a
...
through its own host-provided callable (`Settings.store`, wired by the
engine to `Setting.override_rows`), not through this contract. Settings
are process-global rather than per-protocol, and threading them through
the store would force both memory stores and every test double to fake a
...
the password. For an unknown account, or one whose credentials haven't
been derived (the app derives them at password-set time; bcrypt digests
can't be converted), a store MUST return verifier-*shaped* decoy
material with `account_id: nil` - deterministic per username so the
AUTHENTICATE exchange is byte-identical to a real account's and cannot
be used as a username oracle; the proof against that material never
...
A store holding a `Quarantine` mailbox of flagged malware SHOULD keep it
out of every IMAP operation, not just `LIST`: `select_mailbox`,
`status`, `append`, and any `copy`/`move`/`rename` targeting it by name
resolve to `code: :notfound` (and `create_mailbox` to `:exists`), so a
client can neither read the infected bytes nor move them out, and cannot
tell the name from one that was never created. Review happens in the web
UI. (The in-memory reference store has no quarantine concept, so this is
backend policy, not a shape every store must implement.)
...
raw bytes — but on PostgreSQL the app's adapter matches word-level
against a tsvector (the Dovecot-style FTS trade-off: `TEXT "budget"`
always finds "budget", `TEXT "udge"` may not). On MySQL/SQLite it uses
